# 🚀 OIBSIP — Oasis Infobyte Internship Tasks

This repository contains all tasks completed 
during my Web Development Internship at 
**Oasis Infobyte**.

---

## 👨‍💻 Intern Details
- **Name:** Mohammed Akram R
- **Internship:** Web Development
- **Organization:** Oasis Infobyte

---

## 📁 Tasks

### ✅ Task 1 — Landing Page
- Built a fully responsive landing page
- Technologies: HTML5, CSS3, JavaScript
- 🔗 [Live Demo](https://ak7am-cr7.github.io/OIBSIP/Task1-Landing-Page/)
- 📂 [View Code](./Task1-Landing-Page/)

---

### ✅ Task 2 — Portfolio Website
- Built a personal cloud engineering portfolio
- Technologies: HTML5, CSS3, JavaScript
- Features: Animated Background, Glassmorphism, Contact Form
- 🔗 [Live Demo](https://ak7am-cr7.github.io/OIBSIP/Task2-Portfolio/)
- 📂 [View Code](./Task2-Portfolio/)

---

### ✅ Task 3 — Temperature Converter
- Built a temperature converter with Celsius, Fahrenheit & Kelvin
- Technologies: HTML5, CSS3, JavaScript
- Features: Validation, Scale Bar, Fun Facts, Kelvin Support
- 🔗 [Live Demo](https://ak7am-cr7.github.io/OIBSIP/Task3-Temperature-Converter/)
- 📂 [View Code](./Task3-Temperature-Converter/)

---

## 🛠️ Technologies Used
- HTML5
- CSS3 (Animations, Flexbox, Grid)
- Vanilla JavaScript
- Responsive Design
- GitHub Pages (Deployment)

---

## 🔗 Connect With Me
- [LinkedIn](https://www.linkedin.com/in/mohammed-ak7am-r-/)
- [GitHub](https://github.com/ak7am-cr7)
- [Email](mailto:mdrakram3@gmail.com)
